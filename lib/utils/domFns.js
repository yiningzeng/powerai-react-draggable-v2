...
export function addUserSelectStyles(doc: ?Document) {
  if (!doc) return;
  let styleEl = doc.getElementById('powerai-react-draggable-v2-style-el');
  if (!styleEl) {
    styleEl = doc.createElement('style');
    styleEl.type = 'text/css';
    styleEl.id = 'powerai-react-draggable-v2-style-el';
    styleEl.innerHTML = '.powerai-react-draggable-v2-transparent-selection *::-moz-selection {all: inherit;}\n';
    styleEl.innerHTML += '.powerai-react-draggable-v2-transparent-selection *::selection {all: inherit;}\n';
    doc.getElementsByTagName('head')[0].appendChild(styleEl);
  }
  if (doc.body) addClassName(doc.body, 'powerai-react-draggable-v2-transparent-selection');
}

export function removeUserSelectStyles(doc: ?Document) {
  if (!doc) return;
  try {
    if (doc.body) removeClassName(doc.body, 'powerai-react-draggable-v2-transparent-selection');
    // $FlowIgnore: IE
    if (doc.selection) {
      // $FlowIgnore: IE
      doc.selection.empty();
